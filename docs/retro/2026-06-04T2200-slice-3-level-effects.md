...
| `damageSurvived` uses *actual* damage (post-modifier), not *nominal* amount | A level-1 attacker hitting a level-6 target deals 50 not 100; the character survived 50, not 100. More accurate. |
...
| Killing blow does not increment `damageSurvived` | Spec: "Characters that *survive* X damage points gain a level" — you didn't survive it |
